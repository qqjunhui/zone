...
class tree{
    public  $str="";
    function getTree($pid,$step,$flag,$db,$table,$currentid=""){
        $currentPid=null;
        if($currentid!=""){
        ...
        while ($row=$result->fetch_assoc()) {
            ...
            if($id==$currentPid) {
                $this->str .= "<option class='col-xs-10' value='{$id}' selected>{$str}{$catname}</option>";
            }else{
                $this->str .= "<option class='col-xs-10' value='{$id}' >{$str}{$catname}</option>";
            }
